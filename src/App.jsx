...
const API_URL = "https://script.google.com/macros/s/AKfycbycF1BYaRFjuR_Jx_CzpPA7rGRpsuuoVTR47YGmhKiflmTgjYrTeUtpPE-d9MOHZ0fUXg/exec";

const STATUS_CONFIG = {
  Terkirim: { color:"#16a34a", bg:"#dcfce7", icon:"✅" },
  Tunda:    { color:"#d97706", bg:"#fef3c7", icon:"⏸" },
  Kembali:  { color:"#ea580c", bg:"#ffedd5", icon:"🔄" },
  Batal:    { color:"#dc2626", bg:"#fee2e2", icon:"❌" },
  Selesai:  { color:"#2563eb", bg:"#dbeafe", icon:"🏁" },
};
...
const formatStamp = (d) => new Date(d).toLocaleString("id-ID",{
...
function burnTimestamp(file, label, callback) {
  ...
  reader.onload=(e)=>{
    ...
    img.onload=()=>{
      ...
      const now=new Date(), l1=formatStamp(now), l2=label;
      const pad=Math.max(10,img.width*0.022), fs=Math.max(14,Math.floor(img.width*0.032));
      ctx.font=`bold ${fs}px monospace`;
      const bw=Math.max(ctx.measureText(l1).width,ctx.measureText(l2).width)+pad*2;
      const bh=fs*2+pad*2.5, bx=img.width-bw-pad, by=img.height-bh-pad;
      ctx.fillStyle="rgba(0,0,0,0.6)"; ctx.beginPath(); ctx.roundRect(bx,by,bw,bh,6); ctx.fill();
...
// ── Kategori selector (3 chip) ────────────────────────────────
function KategoriSelector({value,onChange,hasError}) {
  const colors={
    "Keramik":   {sel:"#1B2A4A", selBg:"#1B2A4A", icon:"🧱"},
    "Non-Keramik":{sel:"#0f766e", selBg:"#0f766e", icon:"📦"},
    "Campuran":  {sel:"#7c3aed", selBg:"#7c3aed", icon:"🔀"},
  };
  return (
...
function DriverView({drivers,assDrivers}) {
  ...
  const handleFile=(file)=>{
    if(!file)return;
    if(!driver||!nomorSJ){setError(p=>({...p,photo:"Isi Nama Driver & Nomor SJ dulu."}));return;}
    setError(p=>({...p,photo:""}));setProcessing(true);
    burnTimestamp(file,`${driver}  |  ${preview}`,(url,t)=>{setPhoto(url);setPhotoTime(t);setProcessing(false);});
  };
  ...
  const handleSubmit=async()=>{
    ...
    try {
      ...
      const result=await apiGet({
        action:"addSJ",namaDriver:driver,assDriver:assDriver.join(", "),
        nomorSJ:nn,kategori,km1:toNum(km1),km2:km2?toNum(km2):"",
        jamTiba,status,catatan,
      });
      if(result.success){
        if(photo&&result.id){try{await apiPost({action:"uploadPhoto",sjId:result.id,photoBase64:photo});}catch(_){}}
        setSuccess({driver,assDriver:assDriver.join(" & "),formattedSJ:result.formattedSJ,timestamp:result.timestamp,status,kategori});
        setNomorSJ("");setKategori("");setKm1("");setKm2("");setJamTiba("");
        setStatus("Terkirim");setCatatan("");setPhoto(null);setPhotoTime(null);setAssDriver([]);setError({});
      } else {
  ...
  return (
    <div style={{padding:20}}>
      ...
      {success&&(
        <div style={{background:"#dcfce7",border:"1px solid #86efac",borderRadius:12,
          ...
          <div>
            ...
            <p style={{margin:"2px 0 0",color:"#16a34a",fontSize:12}}>
              {success.kategori} · Status: {success.status}
            </p>
          </div>
        </div>
      )}
      ...
      {/* Jam Tiba di Lokasi */}
      <Field label="🕐 Jam Tiba di Lokasi (opsional)"
        hint="Isi jika sudah tiba di lokasi tujuan">
        <input type="time" value={jamTiba}
          onChange={e=>setJamTiba(e.target.value)}
          style={{...IS,fontSize:16,fontWeight:700,color:jamTiba?"#1B2A4A":"#94a3b8"}}/>
      </Field>
      ...
      <Field label="Catatan (opsional)">
        <textarea value={catatan} onChange={e=>setCatatan(e.target.value)}
          placeholder="Contoh: Tunda karena macet, jalan rusak, dst..."
          rows={3}
          style={{...IS,resize:"none",fontSize:14,lineHeight:1.5}}/>
      </Field>

      {/* Foto */}
      <Divider label="📷 FOTO SURAT JALAN"/>
      ...
      {!photo&&!processing&&(
        <>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:8}}>
            {[["📷","Ambil Foto",cameraRef],["🖼","Dari Galeri",galleryRef]].map(([ic,lb,rf])=>(
              <button key={lb} onClick={()=>rf.current.click()} style={{
                padding:"14px 10px",borderRadius:10,border:"1.5px dashed #94a3b8",background:"#f8fafc",
                cursor:"pointer",fontSize:13,fontWeight:700,color:"#475569",
                display:"flex",flexDirection:"column",alignItems:"center",gap:6,
              }}><span style={{fontSize:26}}>{ic}</span>{lb}</button>
            ))}
          </div>
      ...
      {processing&&(
        <div style={{background:"#f1f5f9",borderRadius:10,padding:16,textAlign:"center",color:"#64748b",fontSize:13,marginBottom:18}}>
          ⏳ Memproses gambar...
        </div>
      )}
      {photo&&!processing&&(
        <div style={{marginBottom:18}}>
          <div style={{position:"relative"}}>
            <img src={photo} style={{width:"100%",borderRadius:10,border:"2px solid #cbd5e1",display:"block"}}/>
            <button onClick={()=>{setPhoto(null);setPhotoTime(null);}} style={{
              position:"absolute",top:8,right:8,background:"rgba(0,0,0,0.55)",color:"#fff",
              border:"none",borderRadius:20,padding:"4px 10px",fontSize:12,fontWeight:700,cursor:"pointer"}}>
              ✕ Hapus
            </button>
          </div>
...
function AdminView() {
  ...
  const openEdit=(sj)=>{
    ...
    setEditKm2(sj.km2?String(sj.km2):"");setEditJamTiba(sj.jamTiba||"");
  };
  ...
  const saveEdit=async()=>{
    ...
    try {
      const r=await apiGet({
        action:"updateStatus",id:editId,status:editStatus,
        catatan:editCatatan,km2:editKm2?toNum(editKm2):"",jamTiba:editJamTiba,
      });
      if(r.success){
        setSjList(prev=>prev.map(sj=>sj.id===editId
          ?{...sj,status:editStatus,catatan:editCatatan,km2:editKm2?toNum(editKm2):sj.km2,jamTiba:editJamTiba}:sj));
        setEditId(null);
      }
    } catch(_){}
    setSaving(false);
  ...
  const KATEGORI_COLOR={"Keramik":"#1B2A4A","Non-Keramik":"#0f766e","Campuran":"#7c3aed"};
  const KATEGORI_BG={"Keramik":"#e8eaf6","Non-Keramik":"#ccfbf1","Campuran":"#ede9fe"};
  ...
  return (
    <div style={{padding:20}}>
      ...
      <input type="text" placeholder="Cari SJ, nama driver, kategori..."
          ...
          {editId===sj.id?(
            <div>
              ...
              <div style={{marginBottom:8}}>
                <label style={{fontSize:12,fontWeight:700,color:"#374151",display:"block",marginBottom:4}}>🕐 Jam Tiba di Lokasi</label>
              ...
              <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between"}}>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap",marginBottom:4}}>
                    ...
                    {sj.kategori&&(
                      <span style={{background:KATEGORI_BG[sj.kategori]||"#f1f5f9",
                        color:KATEGORI_COLOR[sj.kategori]||"#475569",
                        padding:"2px 8px",borderRadius:6,fontSize:11,fontWeight:700}}>
                        {sj.kategori}
                      </span>
                    )}
                    {sj.photoUrl&&<span style={{background:"#f0fdf4",color:"#16a34a",padding:"2px 8px",borderRadius:6,fontSize:11,fontWeight:700}}>📷</span>}
                  </div>
                  ...
                  {sj.jamTiba&&(
                    <p style={{margin:"4px 0 0",fontSize:12,color:"#475569"}}>
                      🕐 Tiba di lokasi: <strong>{sj.jamTiba}</strong>
                    </p>
                  )}
                  {sj.catatan&&(
      ...
      {fullPhoto&&(
        <div onClick={()=>setFullPhoto(null)} style={{
          ...
          <button style={{position:"absolute",top:20,right:20,background:"rgba(255,255,255,0.15)",
            color:"#fff",border:"none",borderRadius:20,padding:"8px 16px",fontSize:14,fontWeight:700,cursor:"pointer"}}>
            ✕ Tutup
          </button>
        </div>
      )}
