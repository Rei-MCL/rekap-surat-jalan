...
const API_URL = "https://script.google.com/macros/s/AKfycbzg_fU_2FZ7kGbPXIy99nexnSi4Zw12Mz4s51TyDEa4h0rE6y7RZo_lIPn1jQmFUA6H8w/exec";
...
// ── Default list (muncul langsung, diupdate dari API kalau terhubung) ──
...
function burnTimestamp(file, driverName, formattedSJ, callback) {
  ...
  reader.onload = (e) => {
    ...
    img.onload = () => {
      ...
      const now=new Date(), l1=formatStamp(now), l2=`${driverName}  |  ${formattedSJ}`;
...
function SelectWithCustom({ options, value, onChange, placeholder, hasError }) {
  ...
  const handleRaw = (e) => { setRaw(e.target.value); onChange(toProperCase(e.target.value)); };
      ...
      {isCustom && (
        <div style={{ marginTop:8, position:"relative" }}>
          <input type="text" value={raw} onChange={handleRaw} onBlur={handleBlur}
            placeholder="Ketik nama (huruf besar otomatis)..." autoFocus
...
// ── Multi select Ass. Driver: dropdown dulu, chip di bawah ────
function MultiSelectDropdown({ options, values, onChange }) {
  const [showCustom, setShowCustom] = useState(false);
  const [customVal, setCustomVal]   = useState("");

  // Hanya tampilkan opsi yang belum dipilih di dropdown
  ...
  const addCustom = () => {
    const proper = toProperCase(customVal.trim());
    ...
    <div>
      ...
      {showCustom && (
        <div style={{ display:"flex", gap:8, marginTop:8 }}>
          ...
          <button onClick={()=>{ setShowCustom(false); setCustomVal(""); }} style={{
            padding:"0 12px", background:"#f1f5f9", color:"#64748b",
            border:"none", borderRadius:10, fontWeight:700, cursor:"pointer", fontSize:13,
          }}>✕</button>
        </div>
      )}
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const result=await apiGet({
        action:"addSJ", namaDriver:driver,
        assDriver:assDriver.join(", "),
        nomorSJ:nn, km1:toNum(km1), km2:km2?toNum(km2):"",
      });
      if (result.success) {
        if (photo&&result.id) {
        ...
        setSuccess({ driver, assDriver:assDriver.join(" & "), formattedSJ:result.formattedSJ, timestamp:result.timestamp });
        setNomorSJ(""); setKm1(""); setKm2("");
        setPhoto(null); setPhotoTime(null); setAssDriver([]); setError({});
      } else {
...
function AdminView() {
  ...
  const openEdit=(sj)=>{ setEditId(sj.id);setEditStatus(sj.status);setEditKet(sj.keterangan);setEditKm2(sj.km2?String(sj.km2):""); };
  const saveEdit=async()=>{
    setSaving(true);
    try {
      const r=await apiGet({ action:"updateStatus",id:editId,status:editStatus,keterangan:editKet,km2:editKm2?toNum(editKm2):"" });
      if(r.success){
        setSjList(prev=>prev.map(sj=>sj.id===editId?{...sj,status:editStatus,keterangan:editKet,km2:editKm2?toNum(editKm2):sj.km2}:sj));
        setEditId(null);
      }
    } catch(_){}
    setSaving(false);
    ...
    <div style={{ padding:20 }}>
              ...
              <div style={{ display:"flex",alignItems:"flex-start",justifyContent:"space-between" }}>
                <div style={{ flex:1,minWidth:0 }}>
                  <div style={{ display:"flex",alignItems:"center",gap:8,flexWrap:"wrap",marginBottom:3 }}>
                    <span style={{ fontSize:13,fontWeight:900,color:"#1B2A4A",letterSpacing:0.3 }}>{sj.nomorSJ}</span>
                  ...
                  {sj.keterangan?<p style={{ margin:"5px 0 0",fontSize:12,color:"#94a3b8",fontStyle:"italic" }}>"{sj.keterangan}"</p>:null}
                </div>
      ...
      {fullPhoto&&(
        <div onClick={()=>setFullPhoto(null)} style={{
          ...
          <button style={{
            position:"absolute",top:20,right:20,background:"rgba(255,255,255,0.15)",color:"#fff",
...
export default function App() {
  ...
  // Hardcoded default — langsung tampil tanpa tunggu API
  const [drivers,    setDrivers]    = useState(DEFAULT_DRIVERS);
  const [assDrivers, setAssDrivers] = useState(DEFAULT_ASS_DRIVERS);

  useEffect(()=>{
    // Update dari API kalau terhubung
    apiGet({action:"getDriverList"})
      .then(r=>{ if(r.success&&r.data?.length) setDrivers(r.data); })
      .catch(()=>{});
    apiGet({action:"getAssDriverList"})
      .then(r=>{ if(r.success&&r.data?.length) setAssDrivers(r.data); })
      .catch(()=>{});
  },[]);
